...
import net.plazmix.practice.classic.scoreboard.ClassicEndingScoreboard;
...
public class ClassicEndingState extends StandardEndingState {
    ...
    public ClassicEndingState(@NonNull GamePlugin plugin) {
    ...
    protected void handleStart() {
        ...
        int wins = PlazmixUser.of(winnerUser.getBukkitHandle()).getDatabaseValue("ClassicDuels", "Wins");

        winnerUser.getCache().set(ClassicGameConstants.CLASSIC_WINS_PLAYER_DATA, wins + 1);
        ...
        GameMysqlDatabase statsMysqlDatabase = plugin.getService().getGameDatabase(ClassicGameStatsMysqlDatabase.class);
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            // Player data insert
            winnerUser.getCache().increment(ClassicGameConstants.CLASSIC_GAMES_PLAYED_PLAYER_DATA);
    ...
    protected void handleScoreboardSet(@NonNull Player player) {
        new ClassicEndingScoreboard(player);
    }
