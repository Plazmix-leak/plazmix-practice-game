...
import net.plazmix.practice.spleef.scoreboard.SpleefWaitingScoreboard;
...
public class SpleefWaitingState extends StandardWaitingState {
    ...
    public SpleefWaitingState(GamePlugin plugin) {
    ...
    protected void handleEvent(@NonNull PlayerJoinEvent event) {
        ...
        if (!teleportedToSpawn1) {
            event.getPlayer().teleport(plugin.getCache().getLocation(SpleefGameConstants.SPAWN_1));
            System.out.println("Teleported to Spawn1");
            teleportedToSpawn1 = true;
        } else {
            event.getPlayer().teleport(plugin.getCache().getLocation(SpleefGameConstants.SPAWN_2));
            System.out.println("Teleported to Spawn2");
        }

        GameSchedulers.runLater(10, () -> {

            new SpleefWaitingScoreboard(getTimerStatus(), event.getPlayer());
    ...
    public void onPlayerFall(PlayerMoveEvent event) {
        if (plugin.getCache().getLocation(SpleefGameConstants.SPAWN_1).getY() > event.getTo().getY()) {
            event.getPlayer().teleport(plugin.getCache().getLocation(SpleefGameConstants.SPAWN_2));
        }
